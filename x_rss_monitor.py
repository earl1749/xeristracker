...
from config.settings import (
    X_USERNAME, X_CHANNEL_ID, X_POLL_SECONDS,
    X_INCLUDE_REPLIES, X_INCLUDE_RETWEETS,
)
from xeris import send_message, get_timestamp

# ── RSS sources (self-hosted first, public fallbacks after) ───────────────────
...
async def _fetch_rss(url: str) -> Optional[str]:
    try:
        async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
            r = await client.get(url, headers=_HEADERS)
        if r.status_code == 200 and "<item" in r.text:
            return r.text
        print(f"   ⚠️  RSS {r.status_code} from {url[:50]}")
        return None
    except Exception as e:
        print(f"   ⚠️  RSS error ({url.split('/')[2]}): {e}")
        return None


async def _fetch_with_fallback(username: str) -> Optional[str]:
...
def _parse_items(xml_text: str) -> list[dict]:
    ...
    try:
        ...
        for item in channel.findall("item"):
            ...
            is_reply   = title.startswith("R to @") or (
                title.startswith("@") and not title.startswith(f"@{X_USERNAME}"))
...
OPTIONAL:
  ...
  X_INCLUDE_REPLIES  = false
  X_INCLUDE_RETWEETS = false
  RSSHUB_URL         = https://your-rsshub.up.railway.app
"""
...
def _pick_newest(items: list[dict]) -> Optional[dict]:
    if not items:
        return None
    return max(items, key=lambda p: (_numeric_id(p.get("id", "")), p.get("timestamp", 0.0)))
...
def _parse_items(xml_text: str) -> list[dict]:
    ...
    try:
        ...
        for item in channel.findall("item"):
            ...
            # Heuristic only; RSS sources are not perfectly consistent
            is_retweet = title.startswith("RT @")
            is_reply = (
...
async def x_post_monitor(db) -> None:
    ...
    print(f"   Poll every {X_POLL_SECONDS}s | replies={X_INCLUDE_REPLIES} | RTs={X_INCLUDE_RETWEETS}")
    ...
    async with httpx.AsyncClient(
        ...
        # ── Seed last-seen ID on first run ───────────────────────────────────
        ...
        while True:
            ...
            try:
                ...
                new_posts.sort(key=lambda p: (p.get("timestamp", 0.0), _numeric_id(p.get("id", ""))))
...
def _should_skip(item: dict) -> bool:
    if item["is_retweet"] and not X_INCLUDE_RETWEETS:
        return True
    if item["is_reply"] and not X_INCLUDE_REPLIES:
        return True
    return False
...
async def x_post_monitor(db) -> None:
    ...
    if state:
        ...
        xml = await _fetch_with_fallback(X_USERNAME)
        if xml:
            items = [i for i in _parse_items(xml) if not _should_skip(i)]
            if items:
                newest = max(items, key=lambda p: (p.get("timestamp", 0), p.get("id", "")))
                last_seen_id = newest["id"]
                await db.upsert_x_watch_state(
                    X_USERNAME, "", last_seen_id, newest.get("created_at", ""))
                print(f"   ✅  Initialized — seeded at post ID {last_seen_id}")
    ...
    while True:
        ...
        try:
            ...
            newest = max(items, key=lambda p: p.get("timestamp", 0))
            last_seen_id = newest["id"]
            await db.upsert_x_watch_state(
                X_USERNAME, "", last_seen_id, newest.get("created_at", ""))
